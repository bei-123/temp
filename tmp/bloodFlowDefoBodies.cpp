...
int main(int argc, char *argv[])
{
    ...
    //////////////////////////////
    //xmlFile["geometry"]["openings"]["sortDirection"].read(openingSortDirection);
    xmlFile["geometry"]["openings"]["type"].read(openingType);
   ...
   sp.shear_rate = (4. * sp.u_p) / D;  // in 1/microsec
    ...
    triangleSet = new TriangleSet<T>(sp.wallStlFileName, DBL);
    const plint extendedEnvelopeWidth = 2;
    const plint blockSize = 20;
    const plint envelopeWidth = 1;  // For standard BGK dynamics.存疑，需要确认的值
    plint referenceDirection = 2;
    ////////////add a double referenceResolution, which equal N in specical direction////////
    plint levelformesh = 0;  //////////这个是网格细化时候用到的，不细化是0；
    //plint resolution = referenceResolution * util::twoToThePower(level);
    T margin=3;
    plint resolution = sp.lz_p/sp.dx_p;    
    plint extraLayer = 0;  
    // 核心转化代码把表面几何mesh转化为TriangleBoundary3D，TriangleBoundary3D结构稍后将被用于分配适当的边界条件。
    ...
    //Array<T, 3> location (3,3,3);
    pcout<< "location0 "<< location[0] <<"  location1 "<< location[1]<<"  location2 "<<location[2]<<endl;

    std::vector<BoundaryProfile3D<T, Velocity> *> inletOutlets;
    
    T uAveLB = sp.u_lb;
    setOpenings(inletOutlets, boundary, uAveLB); //此时已经是格子单位的opening了，在找洞
    pcout <<"inletOutlets "<< inletOutlets.size() <<endl;
    pcout <<"openings "<< openings.size() <<endl;
    ...
    for (pluint i = 0; i < openings.size(); ++i) {
        if (openings[i].inlet) {
            ...
            pcout<< "inletCenter0 "<< inletCenter[0] <<"  inletCenter1 "<< inletCenter[1]<<"  inletCenter2 "<<inletCenter[2]<<endl;
        }
    }
    ...
    //Units3D units(Array<T, 3>(0., 0., 0.), Array<T, 3>(sp.lx_p+margin*sp.dx_p*2, sp.ly_p+margin*sp.dx_p*2, sp.lz_p+margin*sp.dx_p*2));
    Units3D units(Array<T, 3>(0., 0., 0.), Array<T, 3>(sp.lx_p, sp.ly_p, sp.lz_p));
    units.set_periodic(false, false, false);       
    ...
    T omega = 1. / sp.tau_lb;
    Dynamics<T, DESCRIPTOR> *dynamics =  new CompleteRegularizedBGKdynamics<T, DESCRIPTOR>(
            omega);  // 前面定义过
    sp.incompressibleModel = false;

    //存疑，这个部分一定要定义nx，ny,nz但是这个lattice后面用不了
   std::unique_ptr<MultiBlockLattice3D<T, DESCRIPTOR>> lattice =
    ...
    lattice->toggleInternalStatistics(false);  //////关闭所有方向的internalstatistics周期性
    //MultiBlockLattice3D<T, DESCRIPTOR> lattice= * latticetotal;
    // Box3D lbdomain = units.lbDomain();
    Box3D lbdomain = lattice->getBoundingBox();    
    // Discretize the domain with n_ lattice sites per direction
    //这套nx，ny，nz是不是可以给shapeOp进行数据交换，不然这里nx，ny，nz没有先定义
    pluint nx = lbdomain.getNx();
    pluint ny = lbdomain.getNy();
    pluint nz = lbdomain.getNz();
    cout << "Saving a " << lbdomain.getNx() << " by " << lbdomain.getNy() << " by "
          << lbdomain.getNz() << " lattice." << std::endl;
    ...
    Group3D group;
        ...
        //group.add(
               // new MultiBlockLattice3D<T, DESCRIPTOR>(nx, ny, nz, dynamics), "lattice");
                group.add(lattice.get(),"lattice");                
            
        //group.add(  &((MultiBlock3D &) voxelizedDomain.getVoxelMatrix()),"ScalarField3D" );
        //pcout << "=======^^========"<<MultiBlockLattice3D<T, DESCRIPTOR>(nx, ny, nz, dynamics).getBlockName() << endl;

        //group.add(  &((MultiBlock3D &) voxelizedDomain.getVoxelMatrix()),"lattice" );
        
        //group.getLattice<T, DESCRIPTOR>("lattice") = *latticetotal;
        
        
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(0, false);  // x-direction Walls
        //pcout << "latticetotal: " << std::endl;
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(1, false);  // y-direction Walls
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(2, false);  // z-direction

        group.getLattice<T, DESCRIPTOR>("lattice").toggleInternalStatistics(false);
    ...
    MultiScalarField3D<int>  undetermined ((MultiBlock3D &)voxelizedDomain.getVoxelMatrix());
    setToConstant(
        undetermined, voxelizedDomain.getVoxelMatrix(), voxelFlag:: undetermined,
        undetermined.getBoundingBox(), 2);
   
    pcout << "Number of  undetermined cells: " << computeSum( undetermined) << std::endl;
    ...
    MultiNTensorField3D<T> *rhoBarJfield =
        generateMultiNTensorField3D<T>(group.getLattice<T, DESCRIPTOR>("lattice"), extendedEnvelopeWidth, numScalars);
    rhoBarJfield->toggleInternalStatistics(false);
    rhoBarJarg.push_back(rhoBarJfield);
    plint processorLevel = 0;
    integrateProcessingFunctional(
        new PackedRhoBarJfunctional3D<T, DESCRIPTOR>(), group.getLattice<T, DESCRIPTOR>("lattice").getBoundingBox(),  group.getLattice<T, DESCRIPTOR>("lattice"),
   ...
   //group.getLattice<T, DESCRIPTOR>("ScalarField3D");
   //pcout<<"成功获得ScalarField3DScalarField3DScalarField3DScalarField3DScalarField3D" <<endl;
    ...
    OffLatticeBoundaryCondition3D<T, DESCRIPTOR, Velocity> boundaryCondition(
        model, voxelizedDomain, group.getLattice<T, DESCRIPTOR>("lattice"));
    boundaryCondition.insert(rhoBarJarg);

    defineDynamics(
        group.getLattice<T, DESCRIPTOR>("lattice"), voxelizedDomain.getVoxelMatrix(), group.getLattice<T, DESCRIPTOR>("lattice").getBoundingBox(),
        new NoDynamics<T, DESCRIPTOR>, voxelFlag::outside);
    T r=    openings[0].innerRadius;
    Box3D other1(nx-153/200,nx-1,0,ny-1,0,(nz-1)/2-r-3);
    Box3D other2(nx-153/200,nx-1,0,ny-1,(nz-1)/2-r-3, nz-1);
    
    
    //defineDynamics(heavyFluid, Box2D(0, nx - 1, 0, 0), new BounceBack<T, DESCRIPTOR>(rho0));
        
    defineDynamics(
        group.getLattice<T, DESCRIPTOR>("lattice"), other1,   new NoDynamics<T, DESCRIPTOR>);
    defineDynamics(
        group.getLattice<T, DESCRIPTOR>("lattice"), other2,   new NoDynamics<T, DESCRIPTOR>);
    
    //initializeAtEquilibrium(
       // group.getLattice<T, DESCRIPTOR>("lattice"), group.getLattice<T, DESCRIPTOR>("lattice").getBoundingBox(), (T)1., Array<T, 3>((T)0., (T)0., (T)0.0));
    //lattice.initialize();
    //iniLattice(group.getLattice<T, DESCRIPTOR>("lattice"), voxelizedDomain);     


    computePackedRhoBarJ(group.getLattice<T, DESCRIPTOR>("lattice"), *rhoBarJfield, group.getLattice<T, DESCRIPTOR>("lattice").getBoundingBox());
...
// #endif
    ...
    while (true) {
        ...
        if (sp.iT % sp.output_timestep == 0 && discardFlag) {
            ...
            if (printVTKs)
                writeVTK(group.getLattice<T, DESCRIPTOR>("lattice"), sp.dx_p, sp.dt_p, sp.iT);
